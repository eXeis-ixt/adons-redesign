<nav class="relative z-50 h-24 select-none" x-data="{ showMenu: false }">
    <div class="container relative flex flex-wrap items-center justify-between h-24 mx-auto overflow-hidden font-medium border-b border-gray-800 md:overflow-visible lg:justify-center sm:px-4 md:px-2 lg:px-0">
        <div class="flex items-center justify-start w-1/4 h-full pr-4">
            <a href="{{route('home')}}" wire:navigate class="flex items-center py-4 space-x-2 font-extrabold text-gray-900 md:py-0">
                <span class="flex items-center justify-center w-16 h-16 text-white  rounded-full">
                    <img src="https://adons.org/images/Adnos-final-file.png" alt="">
        ...
        <div class="top-0 left-0 items-start hidden w-full h-full p-4 text-sm bg-gray-900 bg-opacity-50 md:items-center md:w-3/4 md:absolute lg:text-base md:bg-transparent md:p-0 md:relative md:flex" :class="{'flex fixed': showMenu, 'hidden': !showMenu }">
            <div class="flex-col w-full h-auto overflow-hidden bg-neutral-950 rounded-lg md:bg-transparent md:overflow-visible md:rounded-none md:relative md:flex md:flex-row">
                ...
                <div class="flex flex-col items-start justify-center w-full space-x-6 text-center lg:space-x-8 md:w-2/3 md:mt-0 md:flex-row md:items-center">
                    <a href="{{route('home')}}" wire:navigate class="inline-block w-full py-2 mx-0 ml-6 font-medium text-left text-white md:ml-0 md:w-auto md:px-0 md:mx-2 lg:mx-3 md:text-center">Home</a>
                    <a href="{{route('feature')}}" wire:navigate class="inline-block w-full py-2 mx-0 font-medium text-left text-white md:w-auto md:px-0 md:mx-2 hover:text-indigo-500 lg:mx-3 md:text-center">Features</a>
                    <a href="{{route('blog')}}" wire:navigate class="inline-block w-full py-2 mx-0 font-medium text-left text-white md:w-auto md:px-0 md:mx-2 hover:text-indigo-500 lg:mx-3 md:text-center">Blog</a>
                    <a href="#_" class="inline-block w-full py-2 mx-0 font-medium text-left text-white md:w-auto md:px-0 md:mx-2 hover:text-indigo-500 lg:mx-3 md:text-center">Contact</a>

                </div>
                <div class="flex flex-col items-start justify-end w-full pt-4 md:items-center md:w-1/3 md:flex-row md:py-0">
                    <a href="#" class="w-full px-6 py-2 mr-0 text-white md:px-3 md:mr-2 lg:mr-3 md:w-auto">Sign In</a>
                    <a href="#_" class="inline-flex items-center w-full px-5 px-6 py-3 text-sm font-medium leading-4 text-black bg-white md:w-auto md:rounded-full hover:bg-gray-20 focus:outline-none md:focus:ring-2 focus:ring-0 focus:ring-offset-2 focus:ring-black">Sign Up</a>
                </div>
            </div>
        </div>
        <div @click="showMenu = !showMenu" class="absolute right-0 flex flex-col items-center items-end justify-center w-10 h-10 bg-neutral-950 rounded-full cursor-pointer md:hidden hover:bg-gray-100">
